...
public class ExcelUtils {
	...
	public static TableHeaderMetaData createTableHeader(String[] parents, String[][] children) {
		...
		for (int i = 0; i < parents.length; i++) {
			...
			if (parents[i].indexOf("(元)") != -1) {
				parentColumn.setColumnType(TableColumn.COLUMN_TYPE_FLOAT_2);
			}
	 ...
	 * @param list 数据集
	 * @param headMeta 表头
	 * @param sheetTitle 标签页标题
	 * @param fields 对象或Map属性数组（注意：顺序要与表头标题顺序对应，如数据集为List<Object[]>，则该参数可以为null）
	 * @return TableData
	 */
	@SuppressWarnings("unchecked")
	public static TableData createTableData(List list, TableHeaderMetaData headMeta, String[] fields, String sheetTitle) {
