...
/**
 * Proxy for a standard Servlet 2.3 Filter, delegating to a Spring-managed bean
 * that implements the Filter interface. Supports a "targetBeanName" filter
 * init-param in <code>web.xml</code>, specifying the name of the target bean in
 * the Spring application context.
 * 
 * <p>
 * <code>web.xml</code> will usually contain a DelegatingFilterProxy definition,
 * with the specified <code>filter-name</code> corresponding to a bean name in
 * Spring's root application context. All calls to the filter proxy will then be
 * delegated to that bean in the Spring context, which is required to implement
 ...
 * invocation of the <code>Filter.init</code> and <code>Filter.destroy</code>
 * lifecycle methods on the target bean, letting the servlet container manage
 * the filter lifecycle.
 * 
 * <p>
 * This class is inspired by Acegi Security's FilterToBeanProxy class, written
 * by Ben Alex.
 * 
...
public class DelegatingFilterProxy extends GenericFilterBean {
	...
	protected void initFilterBean() throws ServletException {
		...
		// early,
		// if possible. If the root application context will be started after
		// this
	...
	public void doFilter(ServletRequest request, ServletResponse response, FilterChain filterChain) throws ServletException, IOException {
		if (((HttpServletRequest) request).getRequestURI().indexOf("processMsg") > -1 || ((HttpServletRequest) request).getRequestURI().indexOf("webService") > -1
				|| ((HttpServletRequest) request).getRequestURI().indexOf("weixin") > -1 || ((HttpServletRequest) request).getRequestURI().indexOf("wxImages") > -1) {
	 ...
	 * stored in the <code>ServletContext</code> before this filter gets
	 ...
	 * 
	 * @return the WebApplicationContext for this proxy, or <code>null</code> if
	 *         not found
	 ...
	 * and calls the standard <code>Filter.init</code> method on it, passing in
