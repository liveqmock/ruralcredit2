...
$(function() {
	...
	window.onresize = function(){
	    setTimeout(function(){
	  	  $('#accountInfoList').resizeDataGrid(140, 20, 0, 0);
	    }, 500);
	};
...
//	showCityCombo("provinceId", "cityId", "districtId");
	...
	$("#accountInfoList").datagrid({
		...
		sortOrder : 'desc',
		rownumbers: true,
		url : serverName + "/accountInfo/selectByPage.do?accountType=0",
		columns : [ [  {
			field : "mobilephone",
			title : "手机号码",
			width : 150
...
//	});
    ...
    departmentComboboxTreeWithPanelWidth("branchNameSearch",false,250);
});
...
function showCityCombo(province, city, district) {
	...
	var ciryId = $("#cityId1").combobox("getValue");
	var districtId = $("#districtId1").combobox("getValue");
//
...
//		branchName : $.trim($("#branchNameSearch").val()),
        departmentId : $("#branchNameSearch").combotree("getValues").join(","),
		accountName : $.trim($("#accountNameSearch").val()),
		account : $.trim($("#accountSearch").val()),
		credentialsNumber : $.trim($("#credentialsNumberSearch").val())
	});
}
...
//	 $("#branchNameSearch").val("");
    $("#branchNameSearch").combotree("setValues","");
...
function searchFuzzy(){
	var fuzzyValue = $.trim($("#fuzzyValue").val());
	$("#accountInfoList").datagrid("load",{
