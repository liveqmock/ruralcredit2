$(function() {
	...
	ajaxSubmit(serverName + "/accountInfo/queryModifyCatdAppByPrimarKey.do", {
		modifyCatdAppId : $("#modifyCatdAppId").val()
	}, function(row) {
	...
	/*拒绝原因*/
	$('#refuseCauseDialog').dialog('close');
});
		...
		ajaxSubmit(serverName + "/accountInfo/getImgAmountByCreditAccountCard.do", {
			...
			if (r) {
				if (r.imgAmount <= 0) {
					...
					/*审批拒绝时新增审批原因*/
					if (type == '2' && !$('#refuseCauseArea').val()){
						$('#_b1').linkbutton('enable'); $('#_b2').linkbutton('enable');
						$('#refuseCauseDialog').dialog('open');
						return;
					}
					if ($.trim($('#refuseCauseArea').val()).length > 500){
						$.messager.alert("提示", "原因字数超出限制(500字以内)");
						return;
					}
					$('#_b1').linkbutton('disable'); $('#_b2').linkbutton('disable');
					wp.buttonDisable();
					ajaxSubmit(serverName + "/accountInfo/approvalModifyCatdApp.do", {
						modifyCatdAppId : $("#modifyCatdAppId").val(),
						accountInfoId : $("#accountInfoId").val(),
						type : type,
						creditapplicationId : $("#creditapplicationId").val(),
						refusecause: $.trim($('#refuseCauseArea').val())
					}, function(b) {
						wp.buttonEnable();
