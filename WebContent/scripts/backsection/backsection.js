...
/*
 * combobox.addDic("receivedType", "receivedType"); 财务类型 combobox.addDic("capitalSource", "capitalSource"); 资金来源
 */
	// combobox.addDic("businessType", "businessType");
	combobox.dicAjax();
	$("#loanList").datagrid({
		url : serverName + "/financeReceiveController/selectBackSectionList.do" + conditions,
		pagination : true,
		rownumbers : true,
...
function toReceive() {
	...
	if (undefined == receiveRecordIds) {
		...
		var url = serverName + "/financeReceiveController/toRecevieFinanceBySettle.do";
