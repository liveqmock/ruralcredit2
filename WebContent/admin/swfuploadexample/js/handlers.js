...
function uploadSuccess(file, serverData) {
	var uploadResult = $.trim(serverData);
	var isSuccess = (uploadResult.indexOf("successed")==0?true:false);
	try {
		if(isSuccess){
			var realName=uploadResult.substring(uploadResult.lastIndexOf(":")+1);
			realName=realName.replace(/^\r\n+|\r\n+$/g,"");
			var fileName = realName.substring(realName.lastIndexOf("_")+1);
			var tr = document.getElementById(file.id);
			tr.style.color="green";
			var bar = document.getElementById(file.id+"_bar");
			bar.parentNode.innerHTML="上传成功<input type='text' style='display:none' name='myStatus' class='uploadFileStatusList' />";
			var delObject = document.getElementById(file.id+"_del");
//			delObject.parentNode.innerHTML="&nbsp;";
			var url = "<a href=\"javaScript:ajaxDeleteUpLoad(\'" + realName + "\');\" >删除</a><input type='text' style='display:none' name='uploadFileList' value='"+realName+"'/><input type='text' style='display:none' name='originalFileList' value='"+fileName+"'/>";
			delObject.parentNode.innerHTML=url;
			...
			//alert(tr.firstChild.nextSibling.innerHTML);
			tr.firstChild.nextSibling.innerHTML = "<a name="+realName+" href=\""+serverName+"/creditgroup/fileDownload.do?fileDownName="+encodeURI(encodeURI(realName))+"\" >" + tr.firstChild.nextSibling.innerHTML +"</a>";
		}else{
			var tr = document.getElementById(file.id);
			tr.style.color="red";
			tr.firstChild.innerHTML="";
			var bar = document.getElementById(file.id+"_bar");
			bar.parentNode.innerHTML="上传失败:"+uploadResult;
			var delObject = document.getElementById(file.id+"_del");
			delObject.parentNode.innerHTML="&nbsp;";
		}
	} catch (ex) {
